...
if __name__ == "__main__":
	...
	GenomeR = fasta2dict('/media/sf_DATA/Stage_UM_ISEM/Puce_57K/genome_IGV/labrax_reverse.fa') # Crée un dictionnaire du génome reverse précédemment créé
			...
			if pb == 'No Stop' :
				...
				for i in range(end+1,end1-2,3): # Recherche des codons stop en aval du gène
					if sequence[i:i+3] in codonStop :
						...
						f2.write('%20s | %20s | %20s | %20s | %20s  |  +\n' %(chromosome,endGTF+1,i+1,i+3,sequence[i:i+3])) # Ecrire dans ce fichier les codons stop trouvés ainsi que leur position
						f2.close() # Fermer le fichier
						listeNoStop.append([chromosome,str(int(endGTF+1)),str(int(endGTF+1)+2),str(i+1),str(i+3)])
				...
				if st != 'NA':
					...
					f1.write('%20s | %20s | %20s | %20s | %20s  |  +\n' %(chromosome,startGTF+1,st+1,en+1,sequence[st:en+1])) # Ecrire dans ce fichier les codons start trouvés ainsi que leur position
...
############################# Anti sens ##############################################
			...
			if pb == 'No Stop' :
				...
				for i in range(length-1-(start-1),length-1-(start-1002),3): # Recherche des codons stop en aval du gène
					if sequence[i:i+3] in codonStop :
						f2 = open(pathF2,"a") # Ouverture du fichier "No Stop"
						f2.write('%20s | %20s | %20s | %20s | %20s  |  -\n' %(chromosome,startGTF +1,str(length-(i+2)),str(length-(i)),sequence[i:i+3])) # Ecrire dans ce fichier les codons stop trouvés ainsi que leur position
						f2.close() # Fermer le fichier
				...
				if st != 'NA':
					f1 = open(pathF1,"a") # Ouverture du fichier "No Start"
					f1.write('%20s | %20s | %20s | %20s | %20s  |  -\n' %(chromosome,endGTF+1,length-(en),length-(st),sequence[st:en+1])) # Ecrire dans ce fichier les codons start trouvés ainsi que leur position
					f1.close() # Fermer le fichier 
					listeNoStart.append([chromosome,str(int(endGTF)+1),str(int(endGTF)+3),str(length-(en)),str(length-(st))])
	...
	linesAnnotation = Annotation.read()
	Annotation.close()
	...
	for elt in listeNoStop:
		linesAnnotation = linesAnnotation.replace(elt[1],elt[3])
		linesAnnotation = linesAnnotation.replace(elt[2],elt[4])
	newAnnotation.write(linesAnnotation)
	newAnnotation.close()
