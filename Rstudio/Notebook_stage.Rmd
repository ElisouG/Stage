...
<p style="text-align:justify";>Puis je me suis lancée dans l'écriture du suivi des troubleshooting sur mon site web.</p>

<p style="text-align:justify";>Je viens d efaire un ticket pour savoir quelle est la taille de mon espace disque sur le cluster.</p>

<p style="text-align:justify";>Je vais tenter de refaire une analyse snpEff. Je ne sais plus combien de temps cela prenait. On verra bien. Du coup le temps est cours mais j'ai un problème avec 14 gènes car snpEff les circularise donc il faut que je traite cela demain en priorité.</p>

A la fin de mes scripts wdl il faut faire une évaluation de ce callset pour cela on peut choisir deux voies : 

* CollectVariantCallingMetrics de Picard  
* et GenotypeConcordance de Picard   
* ou VariantEval de GATK    
* et GenotypeConcordance de GATK   

Comme dit [ici](https://gatkforums.broadinstitute.org/gatk/discussion/6308/evaluating-the-quality-of-a-variant-callset).

## Jour 48 : 04/05/2018

<p style="text-align:justify";>Dès que je suis arrivée j'ai retravaillé ma présentation pour en parler avec Erick et Bruno à la réunion de ce matin.</p>

<p style="text-align:justify";>Réunion avec Erick et Bruno :</p>
<p style="text-align:justify";>Concernant la présentation, il ne faut pas faire quelquechose de compliqué mais quelquechose de simple et clair.</p>
...
<p style="text-align:justify";>Concernant le projet CRECHE de 2016, il faut que je continue la mise en place de GTAK pour trouver les SNPs. Après il faudra que je fasse une analyse de liaison entre les phénotypes et les SNPs trouvés (via les outils de Théo). Pour finir trouver les haplotypes liés via des outils simples.</p>

<p style="text-align:justify";>Pour le run de GATK d'hier : J'avais laissé les runtime en actif et non en commentaire du coup ça ne lui a pas plu. Donc j'ai fait cette modif et j'ai relancé le script.</p>

<p style="text-align:justify";>Je suis allée voir Théo pour qu'il me parle de son analyse RDA (redondance analysis).</p>


## Jour 49 : 07/05/2018

<p style="text-align:justify";>Aujourd'hui, pas grand chose de très productif. J'ai regardé où mon script wdl en était et j'ai découvert qu'il s'était arrêté à MarkDuplicates car  il n'y a plus assez d'espace disque. Si mes calculs sont corrects il devait s'arrêter ce soir vers 17h. Mais n'ayant plus d'espace disue il ne pouvait plus rien faire. Donc je l'ai arrêté. POur voir si toutes les étapes fonctionnent, je l'ai relancé mais avec un seul fichier BAM à traiter à la place de 3. Ça devrait prendre moins d'espace disque sur le serveur et pouvoir tourner jusqu'à la fin. En fin j'espère. J'aimerais que ça ait fini de tourner mercredi pour que je lance la suite du script et voir ce qui fonctionne et ce qui ne fonctionne pas. Ensuite pour le choix des filtres à appliquer à mes données ("hardfiltering") je pense que je vais produire un VariantsToTable pour voir les scores disponibles avant de choisir mes filtres. IL faut aussi que je sache exactement ce qu'ils veulent dire.</p>

<p style="text-align:justify";>Comme j'ai pas trop envie de travailler sur mon script Python j'essaie de faire une digestion in silico du génome du bar via un Package R nommé SimRAD. Pour l'instant j'en suis seulement à l'installation des packages. J'ai réussi à faire fonctionner le package et là je suis après essayer de faire un Rmd avec cela pour en rendre compte à Erick plus tard. En disant que j'avais fait ça avant. Sauf que je n'arrive pas à faire fonctionner knitr.</p>

## Jour 50 : 09/05/2018

<p style="text-align:justify";>J'ai relancé GATK, il a planté lundi soir car je n'avais pas mis le bon chemin pour aller jusqu'au fasta de référence. J'ai donc ajouté le fasta de référence dans le dossier d'inputs de GATK. Sinon l'étape SortSam a été réalisée et semble fonctionner correctement. En fait le problème ne venait pas de là mais surement du fait que les fichiers fasta et son index étaient exécutable au lieu de ne pas l'être. Mais j'ai toujours la même erreur même après ces changements. Je vais voir si j'ai encore une fois la même erreur. Si c'est le cas, il faudra que je vois pour ajouter la création d'un  fichier bai à cahcune des étapes précédentes et suivantes.</p>

<p style="text-align:justify";>Lorsque je regarde les problèmes obtenus avec SnpEff, c'est-à-dire les problèmes de circularisation : ils se produisent avec des gènes que je n'ai pas traité avec mon outils python de recherche de codon strat et stop. La majeure partie des 14 problèmes (11) correspondent à des codons stop qui se trouvent avant la fin du dernier exon. Les 3 autres problèmes sont des problèmes plus importants. Il faut que je regarde si c'est la même chose dans le GTF initial que m'a donné Erick. Ensuite il va falloir que je regarde via IGV pour voir ce qu'il se passe visuellement.</p>

<p style="text-align:justify";>Après il va falloir que je tape mon "discours" pour ma présentation de Lundi prochain pour rassurer Erick et Bruno. J'ai pas trop envie comme Bruno a dit que ça devait être simple vu qu'il s'agit d'une courte présentation afin de savoir qui on est, pourquoi on est là et ce que l'on fait.</p>

<p style="text-align:justify";>Il faudrait aussi que je commence à rédiger mon introduction ainsi que le début de mon matériel et méthodes.</p>



## Jour 51 : 14/05/2018

<p style="text-align:justify";>Aujourd'hui, mail pour demande à propos de ma gratification qui n'a toujours pas été versée. Bruno vient d'appuyer ma demande.</p>

<p style="text-align:justify";>Demande d'augmentation de mon espace disque : demande à Erick et Bruno pour utiliser leur baie de stockage. Bruno va essayer de savoir ce qu'il a besoin de faire pour que j'y accède. Pensez à suivre le ticket de Bruno pour cela.</p>

<p style="text-align:justify";>Mon étape de BaseRecalibrator fonctionne lorsqu'elle est seule mais pas lorsqu'elle est intégrée dans le pipeline. IL faut que je résolve ce problème. Création d'un ticket pour savoir comment cela fonctionne.</p>

<p style="text-align:justify";>Réunion d'équipe pour que tous les stagiaires présentent leurs sujet de stage à l'ensemble de l'équipe ISEM.</p>



## Jour 52 : 15/05/2018

<p style="text-align:justify";>Ce matin transformation des fichiers GTF en fichiers BED. Mais j'ai une erreur qui apparaît: " Genomic end coordinate is less than (or equal to) start coordinate". Certaines de ces erreurs sont les mêmes que celles que SnpEff me donne mais certaines sont en plus (celles là je ne sais pas dire combien elles sont car gtf2bed ne me les donne que 1 par 1). Je pense que ces erreurs viennent du fait que  certaines de mes nouvelles coordonnées de nouveau start et stop sont comprises dans d'autre coordonnées (exemple : 86594 dans 32786594). Cela reste à prouver. Il y a une des erreurs que je dois vérifier sur IGV mais IGV ne veut pas fonctionner aujourd'hui. Je réessairai cet après-midi.</p>

<p style="text-align:justify";>Je vais maintenant m'attaquer au Config file de GATK. Car l'erreur proviendrait du fait que GATK travaille dans des fichiers temporaires différents de ceux dans lesquels PICARD travaille et stocke ces données. Je ne trouve pas comment écrire un fichier config pour GATK. Dons je teste autre chose : Un script de nettoyage des données qui utilise PICARD puis un script de nettoyage bis qui utilise GATK. On va voir si ça fonctionne ainsi. Si ça fonctionne comme ça alors parfait. Je vais décomposer mes scripts en fonction de GATK ou de PICARD. Hier j'avaiss changé le nom de mon dossier pour y supprimer l'accent présent. Sauf que je n'ai pas modifié le fichier d'input.txt. Du coup il ne trouvait pas le dossier avec les accents. Je viens donc de remédier à ce problème. J'ai toujours le même problème qui est : BaseRecalibrator ne trouves pas de fa.fai alors qu'il y est. J'ai ajouté l'option -OBI true. Je vais voir si cela fonctionne mieux ainsi. Ça ne change strictement rien.</p>


## Jour 53 : 16/05/2018

<p style="text-align:justify";>Il faut absolument que je m'occupe des erreurs obtenues après snpEff. Car quand je veux transformer les fichiers GTF en fichiers BED je retrouve les erreurs que snpEff me donne. Je ne comprend pas pourquoi il y a des modif parasites d'une seule extrémité et que le changement de cette coordonnée ajoute une grande séquence. De plus ces erreurs sont absentes des fichiers NoStart et NoStop que j'ai créé. J'ai tout relancé ce matin pour voir d'où ça vient. Mais le script est super long alors qu'il doit âtre court. Je ne sais pas d'où ça vient.</p>

<p style="text-align:justify";>Concernant BaseRecalibrator de GATK, je ne sais pas d'où vient le problème, j'ai rajouté le TMP_DIR de mon choix mais il n'y va pas. J'essaye de relancer le premier script avec la création d'un bam index à la fin de la dernière étape, peut-être que l'erreur vient de là même si il mentionne le fichier index du fichier fasta. On verra bien quand il aura fini.</p>

<p style="text-align:justify";>Comme je ne sais aps travailler avec Python, j'ai lu deux articles cet après-midi sur les GWAS : Forester (RDA de Théo) et PLINK. Maintenant je vais tout copier sur mon disque dur pour essayer de résoudre les problèmes de git ce soir. Plus que 8 minutes à tenir. Je vais écrire un mail à Erick. Puis récupérer les datas et partir.</p>

## Jour 54 : 17/05/2018

<p style="text-align:justify";>Aujourd'hui, j'ai récupéré le fichier GTF qu'Erick a modifié je ne sais comment pour relancer snpEff. J'attends de voir les résultats de snpEff. Il reste 2  gène circulaires à explorer. Il a vérifié que l'extension était correct et en phase puis il a remis les coordonnées initiales.</p>

<p style="text-align:justify";>J'ai lancé BaseRecalibrator seul en direct dans le cluster et ça fonctionne. Du coup je me suis dit que les chemin étaient en dur dans cette option de lancement. Alors pourquoi ça ne marcherait pas dans le script wdl. J'ai donc testé en mettant le chemin du fasta en dur. Résultat : plus de problème avec l'index de ce fichier. Par contre il rencontre le même problème avec le fichier des sites de variations connus. Donc j'ai décidé de le mettre en dur lui aussi. Et résultat : ça tourne de puis maintenant 12 min sans aucun message d'erreur. Je crois que j'ai résolu ce problème!!! Enfin!!!!!!!! Il me reste un problème avec les librairies R qu'utilise AnalyzeCovariate pour faire son plot.pdf pour voir les résultats avant et après la recalibration. Mais ce n'est pas grave car cette étape n'est pas nécessaire pour les étapes suivantes, elle permet juste de vérifier visuellement les recalibrartions qui ont été faites.</p>

<p style="text-align:justify";>J'ai demandé mes vacances et Bruno me les a validées. Reste plus qu'à Erick de les valider. Et il les qa validées lui aussi.</p>

## Jour 55 : 18/05/2018

<p style="text-align:justify";>Sur le forum de GATK ils ont répondu. En fait il suffit de mentionner les fichiers RefIndex et RefDict dans la tâche et dans le call même si l'outils ne l'appelle pas car il va l'utiliser quand même. Je suis en train de tester ce qu'ils m'ont dit. J'espère que ça va marcher. Ce qu'il m'ont dit fonctionne donc je teste maintenant le script suivant qui n'est autre que le script de variant calling.</p>

<p style="text-align:justify";>Dimitri est venu m'installer WinSCP pour que je puisse transférer mes données depuis mon ordinateur jusqu'à la baie de stockage. Sauf que je n'y ai pas encore accès. Normalement Rémy doit y remédier aujourd'hui. De toute façon ce n'est pas urgent mais ça va le devenir si le script fonctionne.</p>

<p style="text-align:justify";>Ce matin j'ai lu 3 papiers sur les statistics des SNPs à savoir des test d'association de variant et des tests d'association d'haplotype. Cet aprèm j'ai lu un papier sur la génétique des pops de *D.labrax*. Et je me suis renseignée sur l'outils GENETIX qu'ils utilisent leur étude. J'ai également fait les ajouts nécessaires sur mon site web.</p>

<p style="text-align:justify";>J'ai essayé de configurer une connection VPN sur l'ordi ubuntu de Flo mais ça ne fonctionne pas car il y a un problème avec des packages R. J'imagine qu'ils ne sont pas à jour. On va voir ça ce soir avec Flo. Il suffisait de faire apt-get update et ap-get upgrade pour tout mettre à jour sous un Ubuntu. Et en plus le logiciel était déjà présent sur son ordi donc je n'avais plus qu'à télécharger l fichier client vpn pour que cela fonctionne correctement.</p>

## Jour 56 : 22/05/2018

<p style="text-align:justify";>Je vais répondre au mail d'Erick cet aprèm comme ça si il passe juste après ça ne me dérangera pas. J'ai peur qu'en lui répondant maintenant il débarque rapidement et je n'ai pas fait grand chose pour ne pas dire rien.</p>

<p style="text-align:justify";>Finalement, mon script python fonctionne. En fait c'est mieux quand on lance le bon script et pas son ancienne version.</p>

<p style="text-align:justify";>Pour GATK, il a tourner pendant 1h50 mais il a rencontré un problème avec GenotypeGVCF car il ne reconnaissait pas l'argument pour la nombre de giga alloué pour cette étape à Java. J'ai donc enlevé les guillemets et relancé le script. Je vais pas tarder à lancer le script alternatif à celui que j'ai écrit pour savoir lequel fonctionne le mieux ^^. Mais il me semble que je dois d'abord finir de l'écrire. Je vais voir ça de suite. Effectivement, il faut que je la peaufine.</p>

<p style="text-align:justify";>J'ai écrit un mail à Erick ainsi qu'un docx pour lui expliquer pourquoi et comment je compte choisir mes filtres pour mes SNPs et mes INDELs. Comme ça il arrêtera de ne pas comprendre de quoi je parle (enfin j'espère).</p>

<p style="text-align:justify";>A la suite de son mail je me suis dit que je pouvais lancer un script similaire sur le cluster qui me permet de récupérer les données pour le choix des filtres plus rapidement. Sauf que toutes les queues sont overbookés à cause de taubier et fgeoffroy qui prennent toutes les noeuds de toutes les queues qui fonctionnent. Donc je dois être patiente et attendre que mes jobs partent.</p>

<p style="text-align:justify";>Madoka va avoir passé la journée entière au labo, ça m'arrange comme ça Erick ne peut pas passer au bureau me demander où j'en suis et ce que je faistout le temps.</p>

<p style="text-align:justify";>Rien à voir avec le taf mais j'ai fait une demande CAF pour savoir pourquoi je n'ai plus aucun droit, j'ai pris RDV pour la vidange, j'ai remboursé la CAF pour le mois de Février perçu en trop (Au moins j'aurai essayé).</p>

## Jour 57 : 23/05/2018

<p style="text-align:justify";>Erick est passé ce matin, on a parlé des filtres pour faire la sélection des véritables SNPs. C'est bien de faire ce que je compte faire même si il semble ne pas avoir tout compris ^^. Il faut absolument que je comprenne la génétique qui est derrière les filtres que je vais choisir pour ne pas choisir n'importe quoi. De plus, on aura un biais dans la répartition des SNPs car on trvaille avec des familles et non des individus totalement indépendants. Je continue de mettre en place le VariantCalling de GATK.
Il faut que je résolve ce pb avec la baie de stockage pour pouvoir lancer des analyses à plus grande échelle (j'entends par là: sur plus de bam).
Pour mon script python, il faut que j'examine les longueurs ajoutées peut-être sous forme de graph pour voir la taille ajoutée aussi bien en start qu'en stop. Il faut qu'on continue ce qu'on a commencé avec Flo. et il faut que je vois pour faire une traduction des CDS avec les ajouts pour voir si la protéines en entière ou tronquée par la présence de codon stop au milieu; Si il y a des codons stop au milieu cela signifiera que les "frame" prises en compte ne sont pas les bonnes. et qu'il faudra y remédier car il est nécessaire d'avoir le bon ORF (cadre de lecture) pour chaque CDS.
Concernant le rapport, il faudrait que je teste différente chose pour voir si la méthodologie fonctionne bien. (avec de gros et de petits bam, avec des réplicas de manip) pour voir si toutes ces possibilités impactent le fonctionnement de GATK.</p>

<p style="text-align:justify";>Pour mon script Python, je me suis occupée de commencer à écrire le script pour parser le fichier gtf du transcriptome que m'a donné Erick. Je vais demander à Flo pour récupérer le fichier au format tabular et non au format visuellement lisible pour pouvoir examiner la longueur que j'ajoute car ça fait plusieurs fois qu'érick me le demande et qu'à chaque fois j'oublies. Donc il faudrait que j'ai fait cela pour demain.</p>

<p style="text-align:justify";>Je pense que je vais me faire une fiche en RMD pour tout ce qui est GATK et j'y ajouterait le choix des filtres. Ou alors je détaille plus GATK et je me fais une fiche à part pour les filtres. Je pense que la deuxième option est la meilleure. C'est celle que je vais choisir pour ne pas faire du double emploi quoi que, une page script et leur détail peut être utile. Finalement j'ai choisi de supprimer la page tools used pour la remplacer par Scripts dans le quel je décrit tout pour mes scripts.</p>

<p style="text-align:justify";>Cet aprèm je serai plus tranquille pour plusieurs raisons : Erick ne risquera pas de débarquer puisqu'il est passé ce matin et Madoka ne sera pas là car elle va maniper au bâtiment 24.</p>

<p style="text-align:justify";>Comme ma carte multi-services ne me permet plus d'ouvrir la porte de l'ISEM, j'ai écrit un mail à la logistique. J'y suis donc allée et elle m'a dit que mon numéro de carte était mentionné 2 fois et que cela venait peut-être de cela. En retournant au bâtiment 22, j'ai testé ma carte à nouveau mais cela n'a rien changé. Elle ne fonctionne toujours pas. J'ai donc écrit un second mail pour dire que ça ne fonctionne toujours afin qu'elle voit avec le service informatique d'où vient le problème. Comme ça ne vient pas du service informatique, j'ai envoyé un mail au responsable de la porte.</p>

## Jour 58 : 24/05/2018

<p style="text-align:justify";>Pour la porte, en fait il fallait que je demande à Yannick et pas à la logistique pour que l'accès fonctionne car Yannick s'occupe de la porte côté ISEM et son collègue de la porte côté Géosciences. Or celle qui ne fonctionne plus c'est celle de l'ISEM.</p>

<p style="text-align:justify";>En fait, j'ai juste à l'ouvrir avec read.table() dans R pour que ça fonctionne. Maintenant il ne me reste plus qu'à faire les graphs. Mais il faut que je trouve comment faire. J'ai réussi à faire des barplot avec ggplot2. POur les Nostart il n'y a aucun problème de visualisation mais pour les Nostop c'est plus délicat car il y a beaucoup de valeurs différentes d'ajouts de longueurs.</p>

<p style="text-align:justify";>Mon accès à la baie de stockage est résolu : j'ai les droits mais je n'ai plus qu'à trouver comment y accéder. Ensuite il faut que je vois comment y transférer mes données. Puis il faudra que je vois comment faire pour travailler sur les bam qui y seront stockés depuis le cluster.</p>

<p style="text-align:justify";>J'ai également fait une demande d'accès au cluster depuis l'extérieur. Et j'ai reçu tout ce qu'il fallait pour le faire. Reste plus qu'à tester ça.</p>

<p style="text-align:justify";>Une femme est venue me voir pour résoudre un de ses problèmes avec ces fichiers fastq qu'elle ne peut merger. Quand elle est venue, je ne savais pas pourquoi ni comment l'aider. Ensuite elle est repartie et j'ai fait une recherche sur Internet pour savoir comment faire. Et j'ai réussi à trouver 4 possibilités différentes. J'ai donc écrit un mail à Bruno pour qu'il lui transmette car je ne connait pas son nom.</p>

## Jour 59 : 25/05/2018

<p style="text-align:justify";>Mes scripts wdl ont plantés parce que Variants to table ne peut prendre qu'un seul fichier à la fois. Donc dans un premier temps je ne vais récupérer que les SNPs pour voir pour les filtres et plus tard je le referai avec les INDELs. J'espère que cette fois ça va marcher et que mes scripts vont partir rapidement.</p> 

<p style="text-align:justify";>J'ai trouvé un rapport de stage sur internet ou quelqu'un fait quasiment la même chose que moi. Donc je vais m'inspirer de son rapport pour faire le miens de la même manière. J'ai donc écrit un plan ainsi que les consignes à respecter sur mon carnet pour m'en rappeler. Et j'ai ajouté tout ça à mon rapport.Rmd que j'avais déjà commencé en rassemblant des consignes de différents horizons comme nous n'avons que des consignes sur la quantité et l'écriture.</p>

<p style="text-align:justify";>J'essaie de transformer mes données de NoStart et NoStop en "numérique" pour pouvoir faire les graphs en ordre croissant mais pour le moment ça me change mes valeurs au lieu de les transformer^^. Ça y est j'ai réussi avec l'aide de Flo en transformant mes données en string puis en numeric.</p>

## Jour 60 : 28/05/2018

<p style="text-align:justify";>Mes jobs sont partis à 22h vendredi soir. Heureusement que je ne les ai pas attendus. Mettre en commentaires les rawIndels n'a pas fonctionné pour la tâche VariantToTable. Je les ai donc supprimé des deux scripts.</p>

<p style="text-align:justify";>Build snpEff s'est bien déroulé. Reste plus qu'à tester l'annotation de la puce avec les dernières modif d'Erick. Je ne suis pas sûre qu'il m'ai envoyé le bon fichier vu que j'ai toujours les deux même erreurs de cricularisation de snpEff.</p>

<p style="text-align:justify";>Erick a téléphoné. Il faut que je continue mes scripts Python. Pour GATK, il faut que je continue de résoudre les différents problèmes avec VaraintToTable pour pouvoir sortir les graphs et choisir les bonnes valeurs pour les filtres et que ke labce en paralèlle le premier script qui c=fonctionne sur plusieurs bam. Il faut que je fasse le transfert des données sur la baie de stockage. Il faut que je vois avec Rémy comment communiquer entre le cluster et la baie de stockage. Pour R (graph des longueurs ajoutées), il faut que je mette en log les valeurs de mon axes des ordonnées ou que je fasse des groupements de classes. Et sur le fichier html il faut qu'il apparaisse différentes choses, par exemple, le nombre total de valeurs traitées ainsi que des phrases explicatives pour mieux comprendre.</p>

<p style="text-align:justify";>J'ai modifié mon fichier html pour y mettre plus de détail et pour y ajouter des données. Je n'arrive pas à passer en log() mon graph des NoStop du coup je suis passée par le second graph pour passer les valeurs de count en log.</p>

<p style="text-align:justify";>J'ai lancé mon script python de correction avec le transcriptome mais il me crée des fichiers vide. Apparemment, j'ai un problème dans mes NoStart NoStop.</p>

## Jour 61 : 29/05/2018

<p style="text-align:justify";>VariantToTable a planté parce que apparemment quand on mets un commentaires dans une ligne de commande il ne voit pas ce qu'il se passe après. Du coup, il pensait que je ne lui avait pas donné d'output. Et HardfilterIndels a planté parce que j'vais remplacé tous les name par samplename et du coup ça avait modifié  un argument de  la fonction. J'avais pourtant regardé pour tout corriger mais cela prouve s=que je n'avais pas tout vu.</p>

<p style="text-align:justify";>Je suis retournée à l'appart rapidement parce qu'on  fait des modifications à mon script hier soir et on a oublié de faire git push. Du coup, j'ai pu récupéré le script faire quelques modifications et tout relancer sur le cluster. Mais mes jobs mettent 100 ans à partir en calcul.</p>

<p style="text-align:justify";>Maintenant je vais m'occuper de l'exo que Flo m'a donné à faire pour m'occuper et lui faire plaisir. C'est fait et tout était quasiment parfait. Je n'ai plus qu'à me trouver qqch à faire pour cet aprèm.</p>

<p style="text-align:justify";>Je vais peut-être leur parler de mon site web. Je vais peut-être creuser PLINK. Enfin bref il faut que je me trouve un truc à faire sinon l'aprèm va être super longue.</p>

<p style="text-align:justify";>Finalement je n'ai rien foutu cet aprèm.</p>

## Jour 62 : 30/05/2018

<p style="text-align:justify";>Mes jobs partent lorsque je ne spécifie pas de queue.</p>

<p style="text-align:justify";>ChoixFiltres a planté sûrement parce que je n'avais pas mis l'option -F qui permet de sélectionner les colonnes que l'on souhaite afficher.</p>

<p style="text-align:justify";>VariantCalling a planté parce qu'il ne reconnaissait pas l'option --java-options. Du coup j'ai mis cette option au tout début juste après java et avant -jar.</p>

<p style="text-align:justify";>On a un petit problème avec mon script python. Les filtres ne fonctionnent pas. Je ne sais pas pourquoi. Mais j'ai pensé à deux possibilités : soit il compare des lettres et non pas des chiffres du coup ça ne peut pas fonctionner soit il y a un problème d'indentation de  "$ if Pass == False". J'ai demandé à Flo, je vais voir ce qu'il en pense. Il pense que ça viens des lettres à la place des chiffres. J'ai donc ajouté la fonction 'int()' à chacune des variables que je déclare. Je me suis également demandé si il ne fallait pas mieux mettre =< à la place de <. Au cas où il serait égal.</p>

<p style="text-align:justify";>J'ai fait un ticket pour demander plusieurs chose: accès à la baie, transfert de data sur la baie et communication entre la baie et le cluster.</p>

<p style="text-align:justify";>J'ai fait une liste de chose que je voudrais que mon (mes) script(s) python fassent :

* Lors de la recherche de codon start ou stop : afficher si il y a des N;       
* A la place de la traduction faire une recherche de stop dans le CDS en fonction des frames;   
* Pour les multiples stop : Noter le frame quelquepart et faire une recherche de stop avec toutes les frames possibles;   
* Pour les non multiple de 3 : Faire une comparaison de la longueur avec le transcriptome et voir pour ajouter ou supprimer des nt en fonction;   
* Modifier le gtf quand tous les tests ont été vérifiés et validés.</p>

## Jour 63 : 31/05/2018

<p style="text-align:justify";>Mon ticket a été résolu en partie. En effet, la connexion ne fonctionnait plus car ils ont eu une coupure de courant. De plus, pour le transfert : je n'étais pas dans le bon dossier mais pour ma défense on ne m'vait pas donné le bon chemin ^^. Pour les calculs sur le cluster avec la baie, je vais devoir réserver une machine big.mem pour pouvoir faire tourner GATK sur l'ensemble de mes Bam.</p>

<p style="text-align:justify";>Pour mon script wdl de Variant_calling j'ai changé de stratégie et supprimer l'étape de GenomicsDBImport pour la remplacer par CombineGVCFs et GenotypeGVCFs. On va bien voir si ça fonctionne.</p>

<p style="text-align:justify";>Erick est content de mon rapport html sur l'évaluation des longueurs ajoutées et la vérification par le transcriptome.</p>

<p style="text-align:justify";>J'ai commencé le script pour vérifier qu'il n'y a pas de stop au sein des CDS que j'ai rallongé avec mon premier script python. Dans une première partie je récupère les positions et les séquences des CDS puis dans une sceonde je les traduit et évalue la présence de stop ou non dans la séquence protéique.</p>

#Journal de bord - Juin

## Jour 64 : 01/06/2018

<p style="text-align:justify";>Erick doit passer ce matin. Du coup, si je trouve des erreurs en me basant sur les données du GTF initial c'est quelles viendront sûrement d'erreurs qu'il y avait déjà au sein de ce GTF initial. Erick prend 3 semaines de vacances en juillet (les 3 dernières semaines). Ca sera la période dans laquelle je devrais commencer à rédiger mon Rapport car il ne sera pas là pour me guider. (Même si au final il ne fait que me donner des pistes et non pas m'aider à programmer). Il vaut mieux que je fasse mon rapport en word car ERick déteste LaTeX qui est un langage que je ne connait pas.</p>

<p style="text-align:justify";>Mon script choixfiltres a fonctionné sans erreur mais le fichier de sortie est vide . Donc je l'ai relancé sans sélection cette fois.</p>

<p style="text-align:justify";>Mon script Variant calling a planté pour la même erreur qu'hier. Je ne sais pas d'où elle vient. J'essaye en sortant CombinedGVCFs de la boucle scatter-gather. Peut-être que le problème vient de là. L'erreur était expected Rbrace ConbinedGVCFs mais je ne sais pas ce que veut dire rbrace.</p>

<p style="text-align:justify";>J'ai commencé un tuto LaTeX pour apprendre ce lana=gage mais c'est un pau complexe de se rappeler de toutes les possibilités.</p>


## Jour 65 : 04/06/2018

<p style="text-align:justify";>Mes scripts wdl ont fonctionné. J'ai rajouté des champs à variantstotable. Mes scripts wdl ne prennent plus que 3h à la place de 17h de temps de calcul.</p>

<p style="text-align:justify";>J'essaye de faire des graphs pour le choix des filtres.</p>

<p style="text-align:justify";>J'ai assisté à la présentation de Théo. C'était bien même si il a pas mal de chose à changer d'ici jeudi et que ça va être court et que Bruno n'est pas là pour l'aider à faire des choix.</p>

## Jour 66 : 05/06/2018

<p style="text-align:justify";>J'ai fait des graph avec les données que j'ai obtenu après haplotypecaller. J'obtiens des Mixed et des Symbolic. Donc ce n'est pas aussi simple qu'avec des SNPs et des INDELs.</p>

<p style="text-align:justify";>Erick m'a envoyé la liste des individus à passer sur GATK en premier et j'ai fait une première sélection parmi ceux-là. Je suis en train de les transférer sur le cluster. Ensuite il faut que je produise le fichier d'index et que je prépare le fichier tsv. Les données ont fini d'être téléchargée mais je ne peux produire leur index car je ne sais pas utiliser samtools du cluster. Donc j'ai chargé les données sur mon disuque dur pour faire ce soir à l'appart. Mais cela ne m'empêche pas de faire le fichier TSV.</p>

<p style="text-align:justify";>Je suis en train de sauvegarder les données que j'ai obtenu avec GATK pour Creche-11GB sur mon ordi pour ensuite les transférer sur la baie de stockage (tycho).</p>

<p style="text-align:justify";>J'ai modifié mon script python pour faire ce que je n'arrivais pas à faire.</p>

## Jour 67 : 06/06/2018

<p style="text-align:justify";>J'ai fais les corrections à mon script python.</p>

<p style="text-align:justify";>Hier soir, j'ai préparé les fichiers .bam.bai et j'ai pu lancer ce matin la préanalyse pour les 8 bam que j'ai sélectionné. J'espère que la mémoire de mon home sur le cluster est bien de 500Go comme il m'a dit sinon ça va planter.^Ca plante parce que je ne lui donne pas assez de ram.</p>

<p style="text-align:justify";>Cet après-midi j'ai rendez-vous avec Erick et Bruno pour parler de l'avancée de mon stage. On se voit tous les 3 environ 1 fois par mois. Bon bah on ne s'est pas vu ^^</p>

<p style="text-align:justify";>A mon fichier rmd j'ai ajouté quelques graphs pour le choix des filtres. Mais erick pense que ça vient de HaplotypeCaller qui ne trouve pas de SNP mais que des mixed ou des symbolic.</p>

## Jour 68 : 07/06/2018

<p style="text-align:justify";>Aujourd'hui c'est mon anniversaire et le cluster de calcul n'est pas accessible donc je ne vais pas pouvoir travailler tant que le disque ne sera pas installé. Et R ne veut pas ouvrir mon projet annotation. Ca y est maintenant il veut bien ^^ . Je ne sais pas pourquoi il ne voulait pas mais maintenant sans que je n'ai rien changé il veut bien, encore un mystère de l'informatique.</p>

<p style="text-align:justify";>Je teste le Pre-processing avec 4 fichiers à la place de 8, pour voir si il peut allouer de la mémoire. Il peut !!!!!!! Du coup c'est trop bien. Je les lance 4/4. Je suis en train de sauvegarder la première partie sur mon ordi. Après je pourrais tout envoyer sur Tycho.</p>

<p style="text-align:justify";>Je teste le choix des filtres avec --output-mode EMIT_ALL_SITES, pour voir si il peut nommer des SNPs.</p>

<p style="text-align:justify";>Sur tycho il n'y a plus le dossier /export/poolz3/bguinand/elise ... J'espère que je n'ai pas fait une fausse manip'. Normalement tout est revenu. Je ne sais pas où tout était parti mais bon, encore un mystère de l'informatique.</p>

<p style="text-align:justify";>J'ai fait ma distribution de gâteaux ausein de l'isem epoiss et au bâtiment 24 pour en donner à la plate-forme. J'en ai profité pour en distribuer aux responsables du cluster. Du coup, j'ai enfin découvert qui était Rémy : il est super jeunne alors que je m'attendais à quelqu'un de plus âgé. En plus, je l'avais déjà croisé plusieurs fois dans les couloirs sans savoir qui c'était. Mais maintenant je le sais et je peux mettre un visage dessus.</p>

<p style="text-align:justify";>J'ai fait un ticket au SR2i pour qu'ils m'installent R et Rstudio sur mon windows car j'aurai accès à 32Go de RAM et au 2To de stockage. Donc ça devrait être plus simple pour moi de travailler mes filtres sur Windows quand ils auront fini bien sûr. Du coup, Dimitri est passé vite fait pour me les installer ppour mon anniversaire sinon je crois qu'il ne serait venu que demain ou plus tard à un moment de creux.</p>


## Jour 69 : 08/06/2018

<p style="text-align:justify";>Hier j'ai testé HaplotypeCaller avec une autre option pour voir si je trouve des SNPs simple sauf que je me suis trompée dans un chemin d'accès à un fichier du coup il n'a pas pu travailler correctement. Donc j'ai fait la correction ce matin et je l'ai relancé. D'autre part, j'avais lancé le PreProcessing sur 4 autres fihciers. Ils ont terminé hier en fin d'aprèm du coup je suis en train de les transférer sur mon ordi. Et après je pourrais les envoyer sur Tycho.</p>

<p style="text-align:justify";>Madoka n'est pas là aujourd'hui ni lundi car elle a pris des jours pour aller en Allemeagne voir sa soeur et son bébé.</p>

## Jour 70 : 11/06/2018

<p style="text-align:justify";>j'ai réservé un bigmem pour mon retour de vacances.</p>

<p style="text-align:justify";>Je ne pouvais plus rien sauvegarder sur le cluster. En fait j'avais plus de place dans mon home (501/500Go) du coup il ne voulais plus rien que je fasse. D'où le temps d'attente super long de mon script. En fait, je crois qu'il n'aurait jamais fini. J'ai tout récupérer grâce à git.</p>

<p style="text-align:justify";>Je ne comprends pas pourquoi Erick ne comprend pas que c'est en log10.</p>

<p style="text-align:justify";>J'ai changer mon script wdl selon les consignes d'ERick. C'est-à-dire produire un fichier bam et faire la soartie des variants lorsque le génotype calling a été réalisé, pour voir d'une part d'où viennent les problèmes d'alignement et d'autre part pour voir si HaplotypeCaller ne sort pas quelquechose de généralisé et que GenotypeGVCF faitle véritable calling final. Ce job est en train de tourner de puis tout à l'heure. J'espère que ça ne va pas être trop long.</p>

<p style="text-align:justify";>J'ai envie d'écrire. Pour assouvir cette envie, je pourrais commencer mon rapport de stage mais je ne suis pas très inspirée pour le moment. Par contre, si jamais je bosse depuis Doué il va falloir que je pense à tout sauvegarder sur mon disque dur si j'en ai besoin jeudi soir avant de partir.</p>

<p style="text-align:justify";>Je ne fais qu'attendre que le temps passe et il ne passe pas vite du coup. J'ai décidé de faire mon sport à l'aide de l'appli que j'ai téléchargé l'autre soir histoire de passer le temps. Du coup, j'ai bien chaud et bien soif. Quand j'aurai fini j'irai chercher de l'eau à la fontaine. Je dois tenir 1 mois. Pour retrouver la forme que j'ai perdu et surtout pour perdre monb petit ventre et mes petites hanches. On va bien voir si ça fait effet ou non.</p>

## Jour 71 : 12/06/2018

<p style="text-align:justify";>Le script wdl a fonctionné : le problème venait bien du fait que les calling d'HaplotypeCaller sont aléatoire et non filtré. Il faut donc faire la sélection des SNPs et INDELs après avoir fait le CombineGVCFs et le GenotypeGVCFs. J'ai fait les graphiques pour l'individus 11GB. Et j'ai lancé l'analyse pour les 4 premiers individus (18, 17, 4, 20B). Quand ça aura terminé je pourrais transférer les données sur Tycho et lancer la suite sur les 4 autres individus (27, 17B, 52, 77).</p>

<p style="text-align:justify";>J'aimerais bien que l'on choisisse les valeurs des filtres avant jeudi soir comme ça je pourrai les appliquer sur toutes les données dès que je rentre de mes vacances sur la big-mem que j'ai réservé et qui me sera attribuée pour 3 semaines entières. J'espère que tout va aller comme sur des roulettes pour la fin de mon stage. En plus pendant 3 semaines voir 4 en juillet je serai libre de faire ce que je veux car Erick sera en vacances et ça sera trop cool. Il veut juste que le premier jet de mon rapport soit dans sa boîte mail avant qu'il ne revienne de vacances pour qu'il n'ai plus que ça à le lire et le corriger en rentant de ses vacances.</p> 

<p style="text-align:justify";>Suite à cette modification du script de choix des filtres, j'ai modifié deux choses: la flowchart sur mon google drive et le script de Variant calling.</p>

<p style="text-align:justify";>D'autre part concernant mon script Python: il produit bien le fichier avec les CDS mais il ne faisait pas de retour à la ligne. J'ai fait ce changement. et J'ai oublié ce que Flo m'avait demandé de faire ensuite. C'était de vérifier que ma condition était bonne et elle ne l'était pas vraiment vu qu'il confondait les tabs avec les espaces. J'ai tout mis sur mon disque dur comme ça on pourra tout regarder ensemble ce soir.</p>

## Jour 72 : 13/06/2018

<p style="text-align:justify";>Erick m'a demandé où j'en était de toutes mes affaires. Donc je lui ai dit que j'avais réservé la bigmem pour mon retour de vacances et sue je pourrais lancer toutes les analyse depuis là-bas dès mon retour de vacances. Je lui ai aussi dit que j'avais quasiment fini de faire la traduction. Et je lui ai également dit ce que je comptait faire avec les autres problèmes de la correction du génome. Je ne sais pas si je vais réussir à faire tout cela en 2 jours seulement.</p>

<p style="text-align:justify";>D'autre part mes script wdl ont planté cette nuit car le séparateur ne séparait pas bien les fichiers si j'ai bien compris. Sinon il faudra que je mette cette étape dans le scatter-gather et peut-être que ça fonctionnera. Mais ça je ne le saurai surement que demain.</p>

<p style="text-align:justify";>Je vais créer tous les bai de tous mes bam à l'aide de mon ordi. Ensuite je vais faire le transfert sur Tycho de tous ça.</p>

<p style="text-align:justify";>J'ai fais une liste de tous ce dont j'aurai besoin pour travailler sur la big-mem. Ainsi qu'une question à poser. Me demande de réservation est toujours en cours de modérartion. Du coup j'ai fait un changement en mettent mon nom comme titrecomme tous le monde.</p>

<p style="text-align:justify";>J'ai fait des recherches sur tout les packeges R que j'avais trouvé</p>

## Jour 73 : 14/06/2018

<p style="text-align:justify";>Vivement ce soir car c'est les vacances !!!!!!!!</p>

<p style="text-align:justify";>Mon wdl a fonctionne trop bien! J'ai donc transférer toutes les données sur mon ordi puis sur Tycho. Je vais pouvoir charger le fichier dans R et voir si il y a beaucoup de différence à les faire seuls ou à plusieurs. J'ai lancé la suite des analyses sur les autres bam que j'avais choisi. Cette analyse s'est terminé rapidement et j'ai donc tout transférer sur mon ordi puis sur Tycho comme d'hab. Ça change quelques trucs mais surtout les MLEAC et MLEAF.</p>

<p style="text-align:justify";>J'ai créé 2 fichier .map et.ped à l'aide de VCFtools pour pouvoir utiliser PLINK. PLINK produit des fichier text séparé par des espaces et ces fichiers sont ouvrables par R ou HaploView. J'ai ensuite rer-regarder le pipeline d'association du rapport que j'avais trouvé et j'ai noter son pipeline sur mon carnet. Et j'ai également regarder le tuto PLINK et notamment les lignes de commandes utilisées. J'ai créé un script sge avec toutes les commandes dedans et je l'ai lancé sur le cluster. Je ne sais pas ce que ça va donner surtout que je n'ai pas ajouté les phénotypes^^.</p>

<p style="text-align:justify";>si pendant mes vacs je dois faire un ticket pour mon ordi : http://help.isem.intra/index.php?redirect=ticket_1124 .</p>

## Jour 74 : 25/06/2018

<p style="text-align:justify";>Aujourd'hui retour de vacances et je suis déjà KO.</p>

<p style="text-align:justify";>J'ai eu les infos que je voulais concernant l'utilisation de la big-mem. J'ai plus qu'à tout transférer pour tous lancer ensuite. J'ai quasiment tout préparé. Je ne vais avoir plus qu'à installer java8 pour faire fonctionner GATK.</p>

<p style="text-align:justify";>Erick est passé me voir pendant près d'1h pour discuter d'où j'en était dans mon taf et de ce que je prevois de faire avant qu'il ne parte en vacances le 6 Juillet au soir. Il faut que je lance GATK par lignée afin de ne pas perdre les snps qui ne seraient supportés que par une seule lignée et éliminé lors du joint calling (GenotypeGVCFs). Puis je lancerai GATK sur l'ensemble des données pour faire une comparariosn avec le lancement pare lignée.
Pour l'annotation du génome : Il faut que je fasse en parallèlle la recherche d'orf (cadre de lecture) et la recherche de start et stop pour faire d'une pierre deux coups. Et pour ceux qui ne fonctionnerait pas il faudrait que j'ajoute un start artificiel pour qu'il mle fasse la traduction en fonction de différents frame.
Pouyr mon rapport, il ne faut pas que j'oublie d'inclure les chiffres prouvant mon avancée avec le nombre d'erreyurs au départ puis à chaque étape pour voir l'évolution de mon travail.
Il faudrait que je preprenne des vacs pour m'aérer la tête avant d'entamer la rédaction d emon rapport.
Pour GATK comme il n'auront surement pas les données avant que je parte (du projet initail) il va falloir que j'écrive un tuto - mode d'emploi de l'utilisation des mes différents scripts pour qu'ils puissent l'utiliser après mon départ sans gros problèmes d'organisation.</p>

<p style="text-align:justify";>D'autre part Madoka n'est pas là de la semaine car elle est à Perpignan pour se former à l'analyse des données épigénétique et l'obtention de données de dd-RAD. Elle ne rvient que la semaine prochaine; Donc j'ai le bureau rien que pour moi toute la semaine pour faire tout ce que je veux !!!!</p>

<p style="text-align:justify";>J'ai fait un schéma de la vue générale de mon pipeline pour la découverte de variants à partir de données de RNA-seq.</p>

<p style="text-align:justify";>J'ai tout transférer sur la big-mem et j'ai lancé mon job mais je ne sais pas comment faire pour qu'il me rende la main.</p>

## Jour 75 : 26/06/2018

<p style="text-align:justify";>J'ai trouvé comment faire pour qu'il me rende la main : c'est "nohup ... &". Il écrit tout ce qui sort dans un fichier nohup.out mais reste à savoir où il le met. Je n'ai pas encore testé mais je vais le faire cet aprèm quand mon script aura fini. Du coup ça ne me rned pas la main.</p>

<p style="text-align:justify";>Comme le script a planté à 20h50 hier soir à cause de la mémoire, j'ai ajouté un argument à ma commande qui est -d64 pour qu'il utilise une JavaVM de 64 bit. Je vais bien voir si ça fonctionne en début d'aprèm. Sinon il va falloir que je joue avec les options -Xms et -Xmx. Le seul pb c'est que je ne sais pas comment faire pour bien choisir les valeurs. Du coup j'ai mis -Xmx4g pour Markduplicates. Et ça fonctionne. Ensuite nouvelle erreur : pas d'index pour le fichier des snps de la puce. Donc je l'ai produit. Et ça m'a donné le même que celui qui était présent. Si ça ne fonctionne pas avec ce petit changement, je l'ajouterai au json et aux task et call du script wdl. En espérant que ça fonctionne sans que j'ai besoin de faire ça.</p>

<p style="text-align:justify";>J'ai fait quelques modifications de mon script pour avoir les tables de recalibration avant et après la recalibration pour voir la différence et l'impact de cette recalibration pour chacun des individus impliqués dans mon étude. Pour l'instant, j'ai lancé mon job avec tous les individus sélectionnés par Erick. Par contre il semblerait que pour faire le joint Calling (GenotypeGVCFs) je doivent le faire par lignée et non tous les individus ensemble.</p>

<p style="text-align:justify";>Je vais attaquer la rédaction de mon rapport, notamment tout ce qui est matériel et méthode ainsi que l'obtention de mes données. Cet aprèm je bosse Python avec l'aide de Flo car il aura fini son oral et sera moins stressé mais pas forcément plus disponible.</p>

## Jour 76 : 27/06/2018

<p style="text-align:justify";>Bon bah mon script wdl a planté pour le même erreur du coup j'ai ajouté le path de ce fichier dans le json et dans mon script wdl. La dernière solution si ça ne fonctionne pas est de mettre le chemin de ce fichier en dur. J'avais fait un oubli et erreur de frappe du coup ça ne lançait pas le job. J'ai fait les modif nécessaires et le job est parti. J'espère que ça va fonctionner cette fois.</p>

<p style="text-align:justify";>Finalement je n'ai pas bossé python de l'aprèm. Je vais faire cela cet aprèm. Pour de vrai cette fois car Flo ne sera pas dispo demain pour m'aider. Il faut que je visualise mentalement ce que je veux faire et comment je compte le faire. Puis j'en parle à Flo et il m'aide et demain je fais tout ce qu'il m'aura dit de faire.</p>

<p style="text-align:justify";>Hier j'ai fait un ticket pour qu'ils viennent m'installer les périphériques d'imprimantes sur mon ordi pour que je puisse imprimer le rapport CRECHE 2016 et avancer plus vite sur la rédaction de mon rapport de stage. Pour mon rapport, j'ai pas beaucoup avancé. J'ai fait le mat etmeth que pour GATK et pour le 1er wdl seulement. Je vais donc m'occuper du second wdl.</p>

<p style="text-align:justify";>Donc PLINK semble fonctionner mais il n'a pas de phénotype et notamment pas de sexe donc c'est difficile pour lui de travailler. Aucun des fichiers ne semble contenir qqch de pertinent car tout est dans nosex.</p>


## Jour 77 : 28/06/2018

<p style="text-align:justify";>Data_PreProcessing a fonctionné. C'est trop cool. Je suis en train de le relancer avec mon script R dans le PATH, j'aimerais bien que ça fonctionne. Je l'ai lancé sans avoir installé les packages nécessaires du coup je les installe. J'espère que ce n'est pas trop tard. Je pense que du coup ça n'a pas été pris en compte car il me dit qu'il ne trouve pas les packages que j'ai installé. Du coup, j'ai ajouté le chemin de la library pour qu'il les trouve. au cas où ça ne fonctionnerait pas j'ai ajouté -l DEBUG et je lancerait le script manuellement. Et d'autre part je transfert les données obtenues sur mon ordi. Pour pouvoir ensuite les transférer sur TYCHO. Mais je sens que ça va prendre du temps car le dossier fait dans les 800Go. Oui oui autant !!!!</p>

<p style="text-align:justify";>Il ne faut pas que j'oublie d'aller voir la présentation d'Olivia sur les Gobies au 2ème étage. Bruno avait 11h15 du coup je me suis pointée à 11h05 et Olivia était déjà en train de faire ça présentation. Du coup je n'ai pas pu assister à sa présentation. En plus elle n'a même pas pensé à moi avant de partir manger. Surtout que je l'ai vu à la cafèt mais ils sont tout de suite parti je ne sais où du coup j'ai mangé toute seule dans mon bureau.</p>

<p style="text-align:justify";>Dimitri vient de passer pour m'installer R 3.5.0 et non  3.4.4 car il ne l'a pas et cele là n'est pas dispo pour Ubuntu ^^. Il m'a aussi paramétré l'imprimante mais je ne peux pas imprimer en recto verso du coup ça va faire du gaspillage donc je vais l'imprimer à l'appart. Et en plus ça sera en couleur.</p>

Pour mettre R à jour:   
* $sudo su   
...
<p style="text-align:justify";>Hier j'ai tenté de lancer le script avec -l DEBUG sauf que ce n'est pas une option reconnue. Du coup je l'ai enlevé et j'espère que ça va fonctionner cette fois sinon je laisse tomber. Je suis un peu têtue du coup je pense qu'il y a un truc que je n'avais pas compris. J'imagines qu'il fallait remplacer le fichie .pdf par le fichier .csv et non mettre les deux. En fait l'erreur est le chargement des librairies qui ne peut-être fait. Je ne sais pas d'où vient ce pb car quand je tape mon script R dans R de la big-mem il "fontionne". De plus, j'ai lancé le job avec nohup &. J'espère que cette fois ça va fonctionner et dans le doute je ne vais pas éteindre mon ordi du week-end même si je sais que ce n'est pas bien.</p>

[site mal compris dans un 1er temps](https://gatkforums.broadinstitute.org/gatk/discussion/4294/analyzecovariates-fails-with-error-message-rscript-exited-with-1)

<p style="text-align:justify";>Je viens de préparer un mail pour l'envoyer à Erick avant de partir. J'espère que ce que j'ai fait cette semaine va lui convenir parce que c'est mon unique mail de la semaine. Vivement qu'il soit en vacances pour ne plus me sentir inutile et fénéante ur mon trvail. Surtout que par rapport à Flo j'ai rien fait. Mais pour ma défense j'ai pas de bioinformaticien pour m'aider dans l'écriture de mes scripts ou l'utilisation des outils que j'emploi. Ce qui m'énerve le plus c'est qu'il fait genre qu'il sait tout mais en fait il sait juste que ça existe et pas comment faire. En fait il ne connait que la partie biologie.</p>

<p style="text-align:justify";>J'ai également corriger les erreurs de traduction de la signification des sigles des filtres que je vais employer aussi bien sur le rapport que sur le mode d'emploi de mes scripts. J'ai aussi ajouté quelques trucs au sein de mon matériel et méthode pour étoffer mon rapport.</p>

<p style="text-align:justify";>J'ai de nouveau écrit ce que je veux faire avec mon script Python pour faire la correction de l'annotation en prennnat en compte ce que Erick m'a dit Lundi dernier. J'espère qu'avec Flo on ourra bosser samedi aprèm sur ça. Entre le coiffeur et la préparation de la quiche.</p>
